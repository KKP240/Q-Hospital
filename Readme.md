...
└──────┬───────┘    └──────┬───────┘    └──────┬───────┘    └──────┬───────┘
       ...
       ▼                   ▼                   ▼                   ▼
┌──────────────┐    ┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│  Appointment │    │    Queue     │    │   Payment    │    │     User     │
│   Service    │◄──►│   Patient    │◄──►│    & Pill    │◄──►│   Service    │
│   :8081      │    │   Service    │    │   Service    │    │   :8084      │
│              │    │   :8082      │    │   :8083      │    │              │
│ • สร้างนัดหมาย │    │ • จัดการคิว   │    │ • ชำระเงิน     │    │ • ลงทะเบียน   │
│ • ยืนยันนัด    │    │ • เรียกคิว     │    │ • สั่งยา       │    │ • ล็อกอิน      │
│ • ยกเลิกนัด    │    │ • ปิดคิว      │    │ • จ่ายยา      │    │ • จัดการผู้ใช้   │
└──────┬───────┘    └──────┬───────┘    └──────────────┘    └──────────────┘
       │                   │
       │                   │
       ▼                   ▼
...
│                    Exchange: "hospital" (Topic)                 │
├─────────────────────────────────────────────────────────────────┤
│  Routing Keys:                                                  │
│  • appointment.confirmed ─────► queue-patient-service (Consumer)│
│  • queue.done ────────────────► payment-pill-service (Consumer) │
│  • user.created ──────────────► (Future consumers)              │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌───────────────────────────────────────────────────────────────────┐
...
├─────────────────┬─────────────────┬─────────────────┬─────────────┤
│ appointment_db  │    queue_db     │   payment_db    │  user_db    │
└─────────────────┴─────────────────┴─────────────────┴─────────────┘
